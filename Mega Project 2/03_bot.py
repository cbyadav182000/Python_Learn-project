...
pyautogui.click(1284, 1055)

time.sleep(1)  # Wait for 1 second to ensure the click is registered
while True:
    time.sleep(5)
    # Step 2: Drag the mouse from (661, 125) to (1908, 833) to select the text
    pyautogui.moveTo(972,202)
    pyautogui.dragTo(1908, 833, duration=2.0, button='left')  # Drag for 1 second

    # Step 3: Copy the selected text to the clipboard
    pyautogui.hotkey('ctrl', 'c')
    time.sleep(2)  # Wait for 1 second to ensure the copy command is completed
    pyautogui.click(661, 125)
